...
class Cityscapes_P(Dataset):
    def __init__(self, split, n_points, image_size):
        ...
        if split == "train":
            with open(
                "/kaggle/input/cityscapes-fine-dataset/train_cityscapes.pkl", "rb"
            ) as file:
    ...
    def __getitem__(self, idx):
        ...
        if "train_extra" in img_pth:
            msk_pth = img_pth.replace(
                "leftImg8bit.png", "gtCoarse_labelIds.png"
            ).replace("leftImg8bit", "gtCoarse")
        else:
            msk_pth = img_pth.replace("leftImg8bit.png", "gtFine_labelIds.png").replace(
                "leftImg8bit", "gtFine"
            )
        ...
        if self.augmentation:
            ...
            object_numbers = [
                x
                for x in object_numbers
                if x not in [0, 1, 2, 3, 4, 5, 6, 9, 10, 14, 15, 16, 18, 29, 30, -1]
            ]
